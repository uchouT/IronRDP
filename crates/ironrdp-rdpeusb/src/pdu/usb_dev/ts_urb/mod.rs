...
macro_rules! ensure_no_ack {
    ($direction:expr, $no_ack:expr, $ts_urb_name:expr) => {{
        if matches!($direction, TransferDirection::In) && $no_ack {
            return Err(invalid_field_err!(
                concat!("TRANSFER_IN_REQUEST::TsUrb: ", $ts_urb_name, "::TS_URB_HEADER::NoAck"),
...
impl Encode for TsUrbIn {
    fn encode(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        ...
        match self {
            ...
            CtlTransfer(urb) => {
                ensure_no_ack!(TransferDirection::In, urb.header.no_ack, "TS_URB_CONTROL_TRANSFER");
                ensure_transfer_flag!(TransferDirection::In, urb.transfer_flags, "TS_URB_CONTROL_TRANSFER");
                urb.encode(dst)
            }
            ...
            IsochTransfer(urb) => {
                ensure_no_ack!(TransferDirection::In, urb.header.no_ack, "TS_URB_ISOCH_TRANSFER");
                ensure_transfer_flag!(TransferDirection::In, urb.transfer_flags, "TS_URB_ISOCH_TRANSFER");
                urb.encode(dst)
            }
            ...
            CtlTransferEx(urb) => {
                ensure_no_ack!(TransferDirection::In, urb.header.no_ack, "TS_URB_CONTROL_TRANSFER_EX");
                ensure_transfer_flag!(TransferDirection::In, urb.transfer_flags, "TS_URB_CONTROL_TRANSFER_EX");
                urb.encode(dst)
            }
        }
...
impl Encode for TsUrbOut {
    fn encode(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        ...
        match self {
            CtlTransfer(urb) => {
                ensure_no_ack!(TransferDirection::Out, urb.header.no_ack, "TS_URB_CONTROL_TRANSFER");
                ensure_transfer_flag!(TransferDirection::Out, urb.transfer_flags, "TS_URB_CONTROL_TRANSFER");
                urb.encode(dst)
            }
            BulkInterruptTransfer(urb) => {
            ...
            IsochTransfer(urb) => {
                ensure_no_ack!(TransferDirection::Out, urb.header.no_ack, "TS_URB_ISOCH_TRANSFER");
                ensure_transfer_flag!(TransferDirection::Out, urb.transfer_flags, "TS_URB_ISOCH_TRANSFER");
                urb.encode(dst)
            }
            CtlDescReq(urb) => urb.encode(dst),
            ...
            CtlTransferEx(urb) => {
                ensure_no_ack!(TransferDirection::Out, urb.header.no_ack, "TS_URB_CONTROL_TRANSFER_EX");
                ensure_transfer_flag!(TransferDirection::Out, urb.transfer_flags, "TS_URB_CONTROL_TRANSFER_EX");
                urb.encode(dst)
            }
        }
