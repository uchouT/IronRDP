...
use super::super::value::{ClassCode, TransferType, UsbSpeed};
use super::{
...
impl<'a> ConfigurationDescriptor<'a> {
    ...
    #[must_use]
    pub const fn max_power_milliamps(self, speed: UsbSpeed) -> u16 {
        let unit = if speed.is_superspeed() { 8 } else { 2 };
        self.max_power_raw() as u16 * unit
    }
}
...
impl<'a> ConfigurationDescriptorSet<'a> {
    ...
    pub fn required_length(prefix: &[u8]) -> Result<Option<usize>, DescriptorError> {
        ...
        let total_length = usize::from(le_u16(prefix, 2));
