...
use super::super::value::{ClassCode, UsbSpeed, is_valid_bcd_version};
use super::{
...
impl<'a> DeviceDescriptor<'a> {
    ...
    pub fn validate(self) -> Result<(), DescriptorError> {
        ...
        if !is_valid_bcd_version(usb_version) {
            return Err(invalid_field(
                0,
                descriptor_type::DEVICE,
                DescriptorField::UsbVersion,
                u32::from(usb_version),
            ));
        }
        let device_release = self.device_release();
        if !is_valid_bcd_version(device_release) {
            return Err(invalid_field(
                0,
                descriptor_type::DEVICE,
                DescriptorField::DeviceRelease,
                u32::from(device_release),
            ));
        }
...
impl<'a> StringDescriptor<'a> {
    ...
    pub fn trailing_byte(self) -> Option<u8> {
        ...
        (payload.len() % 2 != 0).then(|| payload[payload.len() - 1])
    }
}
