...
use super::value::{Direction, TransferType};
...
impl IsochronousUsageType {
    ...
    /// Construct a usage type from its unshifted two-bit field value.
    #[must_use]
    pub const fn from_raw(raw: u8) -> Option<Self> {
        if raw <= 0x03 { Some(Self(raw)) } else { None }
    }
...
impl MaxPacketSize {
    ...
    pub const fn high_speed_payload_per_microframe(self) -> Result<u32, MaxPacketSizeError> {
        match self.additional_transactions() {
            Ok(additional) => Ok(self.packet_size() as u32 * (additional as u32 + 1)),
            Err(error) => Err(error),
        }
    }
}
